<template>
  <div>
    ...
    <div v-if="type=='textarea'&& tlposition !='none'&&inputBorder =='all'" class="input-view"
          :class="labelPosition=='top'?'':tlposition=='top'?'flex_y_start':tlposition=='center'?'flex_y_center':tlposition=='bottom'?'flex_y_end':''">
      <text
          :class="labelPosition=='right'?'txt-r':labelPosition=='left'?'txt-l':labelPosition=='center'?'txt-c':labelPosition=='top'?'txt-top':''">
        {{label}}
      </text>
      <div :class="labelPosition=='top'?'w-100':''">
				<textarea :style="{height:height+'px'}" :class="error&&mustFill?'error':error==false?'success':''" :name="name"
                  @blur="input_blur" @focus="getFocus" @input="input" v-if="type=='textarea'" v-model="data"
                  :maxlength="maxlength"
